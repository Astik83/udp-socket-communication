...
int main() {
    ...
    sock.sin_port = htons(5000);  // use htons
    ...
    getsockname(ret, (struct sockaddr *)&receiver_addr, &receiver_len);
    int receiver_port = ntohs(receiver_addr.sin_port);
    printf("Receiver's port number: %d\n", receiver_port);
  ...
  while(1){
    ...
    if(strcmp(str, "exit") == 0) {
        printf("Sender has quit. Exiting.\n");
        break;
    }
    if(ret1 == -1){
    ...
    send.sin_port = htons(7000);  // use htons
    send.sin_addr.s_addr = inet_addr("127.0.0.2");
